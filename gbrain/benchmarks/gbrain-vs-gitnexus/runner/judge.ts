...
interface JudgeConfig {
  apiKey: string;
  model: string;
  /** API base URL override (for OpenAI-compatible APIs like DeepSeek) */
  baseUrl?: string;
}
...
async function callJudge(
  ...
  if (config.baseUrl) {
    ...
    return callJudgeOpenAI(prompt, config);
  }
  return callJudgeAnthropic(prompt, config);
}
...
async function callJudgeAnthropic(
  prompt: string,
  config: JudgeConfig,
): Promise<QualityDimensionScores> {
...
function parseJudgeResponse(text: string): QualityDimensionScores {
